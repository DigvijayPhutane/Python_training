...
while True:
  try:
    ...
    if count ==  3:
      logging.critical('number of attempts are 3 that it go to hell')
      import sys
      sys.exit(0)
    else:
      logging.warning('number of attempts remaiing are :'.format(3-count))

    choice = input('do u want to stop? ')
    if choice == 'y':
      logging.critical("gonna stop program")
      break
